...
% ------------------------------------------------------------------- 
% 1. This is Algorithm 2 [based on recursion (24)] in the following paper:
%    Kulikova M.V. (2020) 
...
% ------------------------------------------------------------------- 
function [PI,predX,predDP] = Chandrasekhar_IMCCKF2(matrices,initials_filter,measurements,handle_kernel)
...
inv_Rek_next = inv(Rek_next);                          % compute inverse matrix of size m, once    
       K_pk  = (K_pk*Rek + F*L*M*L'*H')*inv_Rek_next; 
           M = M - lambda*M*L'*H'*inv_Rek_next*H*L*M;                 
